...
class Chunk
{
        ...
        public function initChunk(Level $level)
        {
                if (!$this->isInit) {
                        ...
                        foreach ($this->NBTtiles as $nbt) {
                                if ($nbt instanceof CompoundTag) {
                                        // Compatibility fix for worlds imported from MCPE 1.1.5
                                        // (and other old formats): old tile NBT may store the "id"
                                        // tag as IntTag (numeric ID) instead of StringTag, and
                                        // coordinates x/y/z as ShortTag instead of IntTag.
                                        // Without conversion, hasTag(..., StringTag::class) fails
                                        // and the tile is skipped — making chests/ender chests
                                        // invisible and unopenable after a world import.

                                        // 1) Convert "id" from IntTag to StringTag.
                                        //    Old format stored numeric tile IDs; we map the
                                        //    known ones to their string equivalents.
                                        $idTag = $nbt->getTag(Tile::TAG_ID);
                                        if ($idTag instanceof IntTag) {
                                                $numericId = $idTag->getValue();
                                                $stringId = self::legacyNumericTileIdToString($numericId);
                                                if ($stringId !== null) {
                                                        $nbt->setString(Tile::TAG_ID, $stringId);
                                                        $changed = true;
                                                } else {
                                                        // Unknown numeric ID — skip this tile.
                                                        $changed = true;
                                                        continue;
                                                }
                                        } elseif (!$nbt->hasTag(Tile::TAG_ID, StringTag::class)) {
                                                $changed = true;
                                                continue;
                                        }

                                        // 2) Ensure "id" string has no "minecraft:" prefix issues.
                                        //    Some old formats stored "Chest", others "minecraft:chest".
                                        //    Both are already registered in Tile::registerTile(),
                                        //    so no conversion needed here.

                                        // 3) Convert x/y/z from ShortTag to IntTag if needed.
                                        //    Old formats used ShortTag for coordinates; getInt()
                                        //    may return 0 for ShortTag on some NBT implementations,
                                        //    causing the tile to be placed at (0,0,0).
                                        foreach ([Tile::TAG_X, Tile::TAG_Y, Tile::TAG_Z] as $coordTag) {
                                                $tag = $nbt->getTag($coordTag);
                                                if ($tag instanceof ShortTag) {
                                                        $nbt->setInt($coordTag, $tag->getValue());
                                                        $changed = true;
                                                }
                                        }

                                        // 4) Try to create the tile, but don't let a single
                                        //    broken tile crash the whole chunk load. Old worlds
                                        //    may have tile NBT with unexpected/missing fields
                                        //    that cause readSaveData() to throw.
                                        try {
                                                if (Tile::createTile($nbt->getString(Tile::TAG_ID), $level, $nbt) === null) {
                                                        $changed = true;
                                                }
                                        } catch (\Throwable $e) {
                                                $level->getServer()->getLogger()->warning("Failed to create tile " . $nbt->getString(Tile::TAG_ID) . " at chunk (" . $this->x . ", " . $this->z . "): " . $e->getMessage());
        ...
        /**
         * Maps legacy numeric tile IDs (used in very old MCPE/PocketMine formats
         * like 1.1.5) to their string equivalents understood by the current
         * TileFactory. Returns null if the numeric ID is unknown.
         *
         * This is used during chunk load to convert old-format tile NBT so that
         * chests, ender chests, signs, etc. remain visible and functional after
         * a world import from an old version.
         */
        protected static function legacyNumericTileIdToString(int $numericId) : ?string{
                static $map = [
                        54 => "Chest",
                        130 => "EnderChest",
                        146 => "Chest",         // trapped chest (treated as chest)
                        61 => "Furnace",
                        ...
                        118 => "Cauldron",
                        52 => "MobSpawner",
                        116 => "EnchantTable",   // enchanting table
                        144 => "Skull",          // head/skull
                        63 => "Sign",            // standing sign
                        68 => "Sign",            // wall sign
                        205 => "ShulkerBox",
                        218 => "ShulkerBox",     // undyed shulker box in old format
                        154 => "Hopper",
                        140 => "FlowerPot",
                        138 => "Beacon",
                        176 => "Banner",         // standing banner
                        177 => "Banner",         // wall banner
                ];
