...
class Level implements ChunkManager
{
	...
	private array $modernConnectionBlockIds = [];
	private array $modernConnectionBlocks = [];
	...
	protected function actuallyDoTick(int $currentTick) : void
	{
		...
		if (($this->server->getNeptuneProperty("level.generic-auto-mob-spawning", false) && $this->gameRules->getBool(GameRules::RULE_DO_MOB_SPAWNING)) && $currentTick % 400 === 0) {
			$eligibleChunks = [];
		...
		foreach ($targetsByProtocol as $protocol => $players) {
			...
			foreach ($resolvedBlocks as $index => $block) {
				...
				if ($protocol >= ProtocolInfo::PROTOCOL_2193) {
					$visualBlock = clone $translated;
					$visualBlock->position(new Position($pk->x, $pk->y, $pk->z, $this));
					$pk->runtimeIdOverride = RuntimeBlockMapping::getInstance($protocol)->toRuntimeIdForBlock($visualBlock);
				}
				$packets[] = $pk;
			}
	...
	public function sendModernBlockConnections(Player $player, int $chunkX, int $chunkZ) : void
	{
		if ($player->getProtocolVersion() < ProtocolInfo::PROTOCOL_2193) {
			return;
		}
		$updates = [];
		foreach ($this->getModernConnectionBlocks($chunkX, $chunkZ) as [$x, $y, $z]) {
			$updates[] = $this->getBlockAt($x, $y, $z);
		}
		$this->sendBlocks([$player], $updates, UpdateBlockPacket::FLAG_ALL);
	}

	private function getModernConnectionBlocks(int $chunkX, int $chunkZ) : array
	{
		$chunkHash = Level::chunkHash($chunkX, $chunkZ);
		if (isset($this->modernConnectionBlocks[$chunkHash])) {
			return $this->modernConnectionBlocks[$chunkHash];
		}
		$chunk = $this->getChunk($chunkX, $chunkZ);
		...
		foreach ($chunk->getSubChunks() as $subChunkY => $subChunk) {
			...
			$hasModernConnectionBlock = false;
			foreach ($subChunk->getBlockLayers() as $blocks) {
				foreach ($blocks->getPalette() as $fullId) {
					$id = $fullId >> Block::INTERNAL_METADATA_BITS;
					if (!isset($this->modernConnectionBlockIds[$id])) {
						$block = BlockFactory::fromFullBlock($fullId);
						$this->modernConnectionBlockIds[$id] = $block instanceof Thin || $block instanceof Stair;
					}
					if ($this->modernConnectionBlockIds[$id]) {
						$hasModernConnectionBlock = true;
						break 2;
					}
				}
			}
			if (!$hasModernConnectionBlock) {
				continue;
			}
			for ($y = 0; $y < Chunk::EDGE_LENGTH; ++$y) {
				for ($z = 0; $z < Chunk::EDGE_LENGTH; ++$z) {
					for ($x = 0; $x < Chunk::EDGE_LENGTH; ++$x) {
						$fullId = $subChunk->getFullBlock($x, $y, $z);
						$id = $fullId >> Block::INTERNAL_METADATA_BITS;
						if (!isset($this->modernConnectionBlockIds[$id])) {
							$block = BlockFactory::fromFullBlock($fullId);
							$this->modernConnectionBlockIds[$id] = $block instanceof Thin || $block instanceof Stair;
						...
						if ($this->modernConnectionBlockIds[$id]) {
							$positions[] = [($chunkX << Chunk::COORD_BIT_SIZE) + $x, ($subChunkY << Chunk::COORD_BIT_SIZE) + $y, ($chunkZ << Chunk::COORD_BIT_SIZE) + $z];
						}
					}
				}
			}
		}
		return $this->modernConnectionBlocks[$chunkHash] = $positions;
	}

	private function invalidateModernConnectionBlocks(int $chunkX, int $chunkZ) : void
	{
		for ($x = -1; $x <= 1; ++$x) {
			for ($z = -1; $z <= 1; ++$z) {
				unset($this->modernConnectionBlocks[Level::chunkHash($chunkX + $x, $chunkZ + $z)]);
			}
		}
	}
	...
	public function createBlockUpdatePackets(array $blocks, int $protocol) : array
	{
		...
		foreach ($blocks as $b) {
			...
			if ($protocol >= ProtocolInfo::PROTOCOL_2193) {
				$packet->runtimeIdOverride = RuntimeBlockMapping::getInstance($protocol)->toRuntimeIdForBlock($fullBlock);
			}
			$packets[] = $packet;
	...
	public function setBlockAt(int $x, int $y, int $z, Block $block, bool $direct = false, bool $update = true) : bool
	{
		...
		try {
			...
			if ($this->getChunk($x >> Chunk::COORD_BIT_SIZE, $z >> Chunk::COORD_BIT_SIZE, true)->setFullBlock($x & Chunk::COORD_MASK, $y, $z & Chunk::COORD_MASK, $block->getFullId())) {
				...
				$chunkHash = Level::chunkHash($chunkX, $chunkZ);
				$this->invalidateModernConnectionBlocks($chunkX, $chunkZ);
				$relativeBlockHash = Level::chunkBlockHash($x, $y, $z);
				...
				if ($direct) {
					...
					foreach (Facing::HORIZONTAL as $side) {
						$updates[] = $block->getSide($side);
					}
					$this->sendBlocks($this->getChunkPlayers($chunkX, $chunkZ), $updates, UpdateBlockPacket::FLAG_ALL_PRIORITY);
