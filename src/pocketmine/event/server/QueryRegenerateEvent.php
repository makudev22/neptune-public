...
class QueryRegenerateEvent extends ServerEvent
{
	...
	public function __construct(Server $server, int $timeout = 5)
	{
		...
		$this->customServerName = $server->getNeptuneProperty("query.server-name", "Neptune");
		$this->motd = $server->getMotd();
		$this->subMotd = $server->getName() . " v" . $server->getPocketMineVersion();
		$this->listPlugins = $server->getNeptuneProperty("query.visible-plugins", true);
		$this->visiblePlayers = $server->getNeptuneProperty("query.visible-players", false);
		$this->plugins = $server->getPluginManager()->getPlugins();
		$this->players = [];
		$onlinePlayers = $server->getOnlinePlayers();
		if ($server->getNeptuneProperty("query.visible-players", false)) {
		...
		$this->map = $server->getNeptuneProperty("query.visible-map", false) ? $server->getDefaultLevel() === null ? "unknown" : $server->getDefaultLevel()->getName() : "world";
