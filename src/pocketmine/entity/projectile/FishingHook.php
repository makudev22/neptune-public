...
class FishingHook extends Projectile
{
	...
	{
		if ($this->getOwningEntity() === $entityHit) {
			return false;
		}
	...
	public function onUpdate(int $currentTick) : bool
	{
		...
		if ($owner instanceof Player) {
			if ($owner->isClosed() || !$owner->isAlive() || !($owner->getInventory()->getItemInHand() instanceof FishingRod) || $owner->distanceSquared($this) > 1024) {
				$this->flagForDespawn();
			}
	...
	public function handleHookRetraction() : void
	{
		...
		if ($this->isValid() && $angler instanceof Player) {
