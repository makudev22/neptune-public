...
class Internet
{
	...
	public static function simpleCurl(string $page, float $timeout = 10, array $extraHeaders = [], array $extraOpts = [], ?\Closure $onSuccess = null) : InternetRequestResult
	{
		...
		curl_setopt_array($ch, $extraOpts + [
			...
			CURLOPT_HTTPHEADER => array_merge(["User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:12.0) Gecko/20100101 Firefox/12.0 " . VersionInfo::NAME . "/" . VersionInfo::BASE_VERSION], $extraHeaders),
			CURLOPT_HEADER => true
		]);
		try {
			$raw = curl_exec($ch);
