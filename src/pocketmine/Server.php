...
class Server
{
	...
	public function loadNeptuneConfig() : void
	{
		$this->deleteSpacesForNickname = (bool) $this->getNeptuneProperty("player.nickname.delete-spaces", true);
		$this->replaceSpacesNickname = $this->getNeptuneProperty("player.nickname.replace-spaces", "_");
		$this->internalErrorKick = $this->getNeptuneProperty("developer.internal-server-error-kick", false);
		$this->customUnknownCommandMessage = $this->getNeptuneProperty("general.custom-unknown-command-message.enabled", false);
		$this->commandFix = $this->getNeptuneProperty("general.command-fix", false);
		$this->keepInventory = $this->getNeptuneProperty("player.keep-inventory", false);
		$this->enableExperimentMode = $this->getNeptuneProperty("player.experiment-mode", true);
		$this->keepExperience = $this->getNeptuneProperty("player.keep-experience", false);
		$this->legacyInventorySystem = $this->getNeptuneProperty("player.legacy-inventory", false);
		$this->folderPluginLoader = $this->getNeptuneProperty("developer.folder-plugin-loader", true);
		$this->mobAiEnabled = $this->getNeptuneProperty("level.enable-mob-ai", false);
		$this->bedrockResourcePacksProtocol = (int) $this->getNeptuneProperty("resource-pack.minimal-bedrock-protocol", ProtocolInfo::PROTOCOL_407);
		$this->cacheStorage = $this->getNeptuneProperty("resource-pack.cache-storage", true);
		$this->minimalLoginProtocol = (int) $this->getNeptuneProperty("player.login.minimal-protocol", 0);
	}
	...
	public function getNeptuneVersion() : string
	{
	...
	public function getNeptuneProperty(string $variable, $defaultValue = null)
	{
		if (!array_key_exists($variable, $this->neptunePropertyCache)) {
			$this->neptunePropertyCache[$variable] = $this->neptuneConfig->getNested($variable);
		}

		return $this->neptunePropertyCache[$variable] ?? $defaultValue;
	}
	...
	public function __construct(
		...
		try {
			...
			$neptuneYmlPath = Path::join($this->dataPath, "neptune.yml");
			if (!file_exists($neptuneYmlPath)) {
				$content = Filesystem::fileGetContents(Path::join(\pocketmine\RESOURCE_PATH, "neptune.yml"));
				@file_put_contents($neptuneYmlPath, $content);
			}
			$this->neptuneConfig = new Config($this->dataPath . "neptune.yml", Config::YAML, []);
			$this->loadNeptuneConfig();
			...
			if (VersionInfo::IS_DEVELOPMENT_BUILD) {
				if (!$this->getProperty("settings.enable-dev-builds", false)) {
					...
					$this->logger->emergency($this->baseLang->translateString("pocketmine.server.devBuild.error5", ["https://github.com/makudev22/neptune-public"]));
			...
			@cli_set_process_title($this->getName() . " " . $this->getNeptuneVersion());
			...
			$this->logger->info($this->getLanguage()->translateString("pocketmine.server.info", [
				$this->getName(),
				(VersionInfo::IS_DEVELOPMENT_BUILD ? TextFormat::YELLOW : "") . $this->getNeptuneVersion() . TextFormat::RESET
			]));
			...
			if ($this->isAllowNether() && $this->getNetherLevel() === null) {
				...
				$netherLevelName = $this->getNeptuneProperty("dimensions.nether.level-name", "nether");
			...
			if ($this->isAllowTheEnd() && $this->getTheEndLevel() === null) {
				...
				$endLevelName = $this->getNeptuneProperty("dimensions.the-end.level-name", "end");
	...
	{
		return (bool) $this->getNeptuneProperty("dimensions.nether.active", true);
	}

	public function isAllowTheEnd() : bool
	{
		return (bool) $this->getNeptuneProperty("dimensions.the-end.active", true);
	}
	...
	public function dispatchCommand(CommandSender $sender, string $commandLine, bool $internal = false) : bool
	{
		...
		$sender->sendMessage($this->customUnknownCommandMessage ? (string) $this->getNeptuneProperty("general.custom-unknown-command-message.message") : $this->getLanguage()->translateString(TextFormat::RED . "%commands.generic.notFound"));
	...
	{
		return $this->getNeptuneProperty("additional-plugin-dirs", []);
	}
