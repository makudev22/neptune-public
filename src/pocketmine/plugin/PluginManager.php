...
class PluginManager
{
		...
		foreach ($loaders as $loader) {
			foreach ($files as $file) {
				...
				try {
					...
					if (isset($plugins[$name]) || $this->getPlugin($name) instanceof Plugin) {
	...
	public function isCompatibleApi(string ...$versions) : bool
	{
		...
		foreach ($versions as $version) {
			...
			if ($version !== $serverString) {
				...
				if ($pluginNumbers[1] === $serverNumbers[1] && $pluginNumbers[2] > $serverNumbers[2]) { //If the plugin requires bug fixes in patches, being backwards compatible
					continue;
				}
			}
	...
	public function registerEvents(Listener $listener, Plugin $plugin) : void
	{
		...
		foreach ($reflection->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
			if (!$method->isStatic() && $method->getDeclaringClass()->implementsInterface(Listener::class)) {
				...
				if ($eventClass === null || !$eventClass->isSubclassOf(Event::class)) {
					continue;
				}
	...
	public function registerEvent(string $event, Listener $listener, int $priority, EventExecutor $executor, Plugin $plugin, bool $ignoreCancelled = false) : void
	{
		...
		$timings = Timings::getEventHandlerTimings($event, get_class($listener), $plugin->getDescription()->getFullName());
